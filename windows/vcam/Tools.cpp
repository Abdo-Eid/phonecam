...
static inline BYTE ClampToByte(int value)
{
...
HRESULT NV12ToRGB32(BYTE* input, ULONG inputSize, LONG inputStride, UINT width, UINT height, BYTE* output, ULONG outputSize, LONG outputStride)
{
	...
	{
		const BYTE* yRow = yPlane + (size_t)h * inputStride;
		const BYTE* uvRow = uvPlane + (size_t)(h / 2) * inputStride;
		BYTE* outRow = output + (size_t)h * outputStride;
